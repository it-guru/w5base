   ...
   if (keys(%tasks)){
      foreach my $pid (keys(%tasks)){
         if (exists($tasks{$pid}->{end})){
            delete($tasks{$pid}) if ($tasks{$pid}->{end}<time()-20);
         }
      }
...
sub async
{
   ...
   my $taskenv={start=>time(),ipc=>new IPC::Smart(-nolocking=>1)};
...
sub callMethod
{
   ...
   if (ref($h) eq "HASH"){
      ...
      eval("\$fres=\$self->$method(\\\%param,\$mux,\$io);");
   }
   if (ref($h) eq "ARRAY"){
      my @param=@{RPC::Smart::ArrayPack($h)};
      eval("\$fres=\$self->$method(\@param);");
   }
   #printf STDERR ("callMethod:$method param=%s\n",Dumper(\%param));
