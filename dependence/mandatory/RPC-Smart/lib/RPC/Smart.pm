...
sub ArrayPack
{
   ...
   for(my $c=0;$c<=$#{$h};$c++){
      ...
      ArrayPack($h->[$c]) if (ref($h->[$c]) eq "ARRAY");
   }
   return($h);
}
...
sub HashPack
{
   ...
   foreach my $k (keys(%$h)){
      delete($h->{$k}) if ($k eq "/order");
      delete($h->{$k}) if ($k eq "/nodes");
      ArrayPack($h->{$k}) if (ref($h->{$k}) eq "ARRAY");
      HashPack($h->{$k})  if (ref($h->{$k}) eq "HASH");
   }
   return($h);
}
...
RPC::Smart - RPC implementation to make simple perl calls on remote sites
