...
int smart_lock(Share *share, int flags) {

  if (share->nolocking) return(-1);
...
Node *_add_segment(Share *share) {
  ...
  while(1) {
    ...
    if (shmctl(node->shmid, IPC_RMID, (struct shmid_ds *) 0) < 0) return NULL;
...
int smart_watched(Share *share) {
   struct shmid_ds shmctl_arg;

   memset(&shmctl_arg,0,sizeof(shmctl_arg));
   if (shmctl(share->head->shmid, IPC_STAT, &shmctl_arg)<0) return -1;
   return(shmctl_arg.shm_nattch-1);
}
...
Share *new_share(key_t key, int segment_size, int flags,int nolocking) {
  Share  *share;
  Node   *node;
  int    semid;
  struct shmid_ds shmctl_arg;
...
again:
  ...
  if ((node->shmaddr = (Header *) shmat(node->shmid, (char *) 0, 0)) == (Header *) -1)
    return NULL;
  if (shmctl(node->shmid, IPC_RMID, (struct shmid_ds *) 0) < 0) 
    return NULL;
  ...
  share->head      = node;
  share->tail      = node;
  share->nolocking = (short) nolocking;
...
unsigned int smart_version(Share *share) {
...
int destroy_share (Share *share, int rmid) {
  ...
  if (rmid) {
    _remove_segments(shmid);
    memset(&shmctl_arg,0,sizeof(shmctl_arg));
    shmctl(shmid, IPC_STAT, &shmctl_arg);
    if (shmctl_arg.shm_nattch==0){
       if (!share->nolocking){
          semctl_arg.val = 0;
...
int smart_num_segments(Share* share) {
  int    count = 0;
  int    shmid;
  Header *shmaddr;
