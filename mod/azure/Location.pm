...
sub new
{
   ...
   $self->setDefaultView(qw(name fullname));
   return($self);
}
...
printf STDERR ("data=%s\n",Dumper($data));
         ...
         foreach my $rawrec (@$data){
            my $rec={};
print STDERR Dumper($rawrec);
            $rec->{name}=$rawrec->{name};
            $rec->{id}=$rawrec->{id};
            $rec->{id}=~s#^/##;
            $rec->{fullname}=$rawrec->{displayName};
            ($rec->{subscriptionId})=$rawrec->{id}=~m#/subscriptions/([^/]+)/#;
            push(@data,$rec);
         }
         return(\@data);
      },
      onfail=>sub{
         ...
         msg(ERROR,$reqtrace);
         $self->LastMsg(ERROR,"unexpected data AZURE Location response");
         return(undef);
      }
   );
