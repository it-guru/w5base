package TS::event::COMigrationToolETCS;
#  W5Base Framework
...
sub COMigrationToolETCS
{
   my $self=shift;
   my $filename=shift;

   msg(INFO,"try to open file '$filename'");
   $ENV{REMOTE_USER}="service/ETCS-Project";
   ...
   if ($total>0){
      ...
      if (keys(%to)!=0){
         $wfa->Notify("INFO",
           ...
           "aufgrund einer notwendigen Umstellung von Kontierungsobjekten ".
           "durch das ETCS Projekt (TelekomIT) ".
           ...
           "Bei Rückfragen zu dieser Migration wenden Sie sich bitte ".
           "an Fr. Vogel Heide (CC).\n".
           "\n".
           "\n".
           "               ---------------------------------------\n".
           "\n".
           ...
           "Because of the necessary migration by ETCS Project (TelekomIT) ".
           "corrections in W5Base/Darwin were ".
           ...
           "view. In case of further questions please contact ".
           "Ms. Vogle Heide (CC).\n "
           ,
           emailto=>[keys(%to)],
           emailcc=>[keys(%cc)]);
