...
sub DataCollector
{
   ...
   my $query=$self->decodeFilter2Query4AWS("RouteTable",$filter);
   if (!defined($query)){
   ...
   try {
      ...
      do{
         ...
         if (exists($query->{id}) && $query->{id} ne ""){
            $param{RouteTableIds}=[$query->{id}];
         }
         else{
            $param{MaxResults}=20;
         }
         my $objItr=$obj->DescribeRouteTables(%param);
         if ($objItr){
            my $RouteTables=$objItr->RouteTables();
            if ($RouteTables){
               foreach my $routetable (@$RouteTables){
                  my %tag;
                  foreach my $tag (@{$routetable->Tags()}){
                  ...
                  my $rec={
                      ...
                      name=>$tag{Name},
                      tags=>\%tag,
                      idpath=>$routetable->{RouteTableId}.'@'.
                              $AWSAccount.'@'.
                              $AWSRegion,
                  };
                  my $Routes=$routetable->Routes();
                  my @gateways;
                  my @routes;
                  foreach my $route (@$Routes){
p $route;
                     my $dest=$route->DestinationCidrBlock();
                     if ($dest ne ""){
                        push(@routes,"$dest");
                     }
                     my $gw=$route->TransitGatewayId();
                     if ($gw ne ""){
                        #my $gwItr=$obj->DescribeTransitGateways(
                        #   TransitGatewayIds=>[$gw]
                        #);
                        #printf STDERR ("gw:\n");
                        #p $gwItr;
