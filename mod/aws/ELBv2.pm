...
sub DataCollector
{
   ...
   my $query=$self->decodeFilter2Query4AWS("ELB",$filter);
   if (!defined($query)){
   ...
   try {
      ...
      do{
         ...
         if (exists($query->{id}) && $query->{id} ne ""){
            $param{LoadBalancerArns}=[$query->{id}];
         }
         my $objItr=$obj->DescribeLoadBalancers(%param);
         if ($objItr){
            my $Elbs=$objItr->LoadBalancers();
            if ($Elbs){
               foreach my $elb (@$Elbs){
                  my $lbarn=$elb->{LoadBalancerArn};
                  my $lobj=$obj->DescribeListeners(LoadBalancerArn=>$lbarn);
                  my $listeners=$lobj->Listeners();
                  my @port;
                  foreach my $listener (@$listeners){
                     push(@port,$listener->Port());
                  }
                  my $rec={
                      id=>$lbarn,
