...
sub new
{
   ...
   $self->AddFields(
      ...
      new kernel::Field::Text(
                name          =>'name',
                label         =>'Software',
                dataobjattr   =>'sw.disp_software_name'),

      new kernel::Field::Text(
                name          =>'version',
                label         =>'Software Version',
                dataobjattr   =>'sw.version'),

      new kernel::Field::Date(
                name          =>'scandate',
                timezone      =>'CET',
                label         =>'last Scandate',
                dataobjattr   =>'sw.last_monitor_date'),
   ...
   $self->setDefaultView(qw(systemname name version scandate));
...
{
   my $self=shift;
   my $from="dina_schema.dina_software_info_vw sw ".
            "join d2dw_system_config_vw cfm ".
            "on  cfm.host_id=sw.host_id ".
...
sub initSqlWhere
{
   ...
   my $where="current_date between cfm.valid_from and cfm.valid_to ".
             "and sw.last_monitor_date > current_date-3";
   return($where);
}
