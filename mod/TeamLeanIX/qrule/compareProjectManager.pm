package TeamLeanIX::qrule::compareProjectManager;
#######################################################################
=pod
...
The Project Manager of an IT application is leading entered in T.EAM via the ICT-Object and is only compared for the production environments in Darwin (valid for CI-States: "available/in project" and "installed/active")
...
Der Projektmanager IT-Applikation ist federführend in T.EAM über das ICTO Objekt gepflegt und wird nur für die Produktionsumgebungen in Darwin abgeglichen (gilt für CI-Status: "verfügbar/in Projektierung" und "installiert/aktiv")
      ...
      $dataobj->NotifyWriteAuthorizedContacts($rec,undef,{
       #  emailcc=>['11634953080001'],
         ...
         if (!$pmexists){    # Wenn in Darwin kein Projektmanager erfasst, dann
            $autocorrect=1;  # darf der PM direkt aus T.EAM übernommen werden.
