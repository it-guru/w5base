package TeamLeanIX::qrule::compareApplMgr2;
#######################################################################
=pod
...
the ICTO Object on T.EAM with contacts in BusinessApplication.
...
The "ApplicationManager deputy" is maintained in T.EAM via the ICTO object as "Application Manager Deputy" and is only adjusted for the production environments in Darwin (valid for CI states: "available/project" and "installed/active").
...
Der ApplicationManager Vertreter ist federführend in T.EAM über das ICTO Objekt als "Application Manager Deputy" gepflegt und wird nur für die Produktionsumgebungen in Darwin abgeglichen (gilt für CI-Status: "verfügbar/in Projektierung" und "installiert/aktiv")
...
sub qcheckRecord
{
   ...
   if ($rec->{opmode} eq "prod" && $rec->{ictono} ne ""){
      ...
      $dataobj->NotifyWriteAuthorizedContacts($rec,undef,{
         ...
         if (defined($parrec)){
            ...
            foreach my $r (@{$parrec->{contacts}}){
               if (($r->{role} eq "Application Manager Deputy" ||
                    $r->{role} eq "Application Manager Deputy - Cape") &&
   ...
   if ($#dataissue!=-1){
      $errorlevel=3;
      unshift(@dataissue,"different values stored in T.EAM:");
      push(@qmsg,@dataissue);
   }
