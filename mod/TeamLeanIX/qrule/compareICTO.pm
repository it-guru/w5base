package TeamLeanIX::qrule::compareICTO;
#######################################################################
=pod
...
This quaity rule compares the specified ICTO ID to T.EAM. A DataIssue
...
IT architecture by means of the tool T.EAM.
...
and is not configured in T.EAM (i.e. pure technical applications 
...
default is not provided from T.EAM. In this case please 
...
liegen, dass diese Default nicht aus T.EAM zur Verfügung 
...
sub qcheckRecord
{
   ...
   if ($rec->{ictono} ne ""){
      ...
      return(undef,undef) if (!$par->Ping());
      if (!defined($parrec)){
         my $msg="the given ICTO-ID does not exist anymore in T.EAM";
         push(@qmsg,$msg);
         push(@dataissue,$msg);
         $errorlevel=3 if ($errorlevel<3);
      }
         ...
         if (lc($parrec->{lifecycle_status}) eq lc("EndOfLife")){
            my $retiredReached=1;
            if ($retiredReached){
               my $msg="the given ICTO-ID is marked as EndOfLife in T.EAM";
               push(@qmsg,$msg);
               push(@dataissue,$msg);
               $errorlevel=3 if ($errorlevel<3);
            }
