package itil::MyW5Base::myclosedincidents;
#  W5Base Framework
...
{
   my $self=shift;
   my $timelabel=$self->getParent->T("Incident end time");;
...
%StdButtonBar(deputycontrol,teamviewcontrol,print,search)%
EOF
   return($d);
}
...
sub Result
{
   ...
   if ($dc eq "TEAM"){
      my %grp=$self->getParent->getGroupsOf($ENV{REMOTE_USER},
                             ["REmployee","RBoss","RApprentice","RFreelancer"],
                              "down");
      my @grpids=keys(%grp);
      @grpids=(qw(-1)) if ($#grpids==-1);
   ...
   $self->{DataObj}->SetNamedFilter("REST",\%q1);
