...
sub new
{
   ...
   $self->AddFields(
      ...
      new kernel::Field::Link(
                name          =>'networkid',
                label         =>'NetworkID',
                dataobjattr   =>'lnknfsnasipnet.network'),
                                                   
      new kernel::Field::Text(
                name          =>'name',
                htmlwidth     =>'100px',
                label         =>'IP',
                dataobjattr   =>'lnknfsnasipnet.ip'),
...
#                onRawValue    =>\&getApplData),
      ...
      new kernel::Field::Text(
                ...
                dataobjattr   =>'lnknfsnasipnet.exportoptions'),

      new kernel::Field::Text(
                name          =>'comments',
                label         =>'Comments',
                dataobjattr   =>'lnknfsnasipnet.comments'),

      new kernel::Field::Creator(
                name          =>'creator',
                group         =>'source',
                label         =>'Creator',
                dataobjattr   =>'lnknfsnasipnet.createuser'),
                                   
      new kernel::Field::Owner(
                name          =>'owner',
                group         =>'source',
                label         =>'last Editor',
                dataobjattr   =>'lnknfsnasipnet.modifyuser'),
                                   
      new kernel::Field::Text(
                name          =>'srcsys',
                group         =>'source',
                label         =>'Source-System',
                dataobjattr   =>'lnknfsnasipnet.srcsys'),
                                                   
      new kernel::Field::Text(
                name          =>'srcid',
                group         =>'source',
                label         =>'Source-Id',
                dataobjattr   =>'lnknfsnasipnet.srcid'),
                                                   
      new kernel::Field::Date(
                name          =>'srcload',
                group         =>'source',
                label         =>'Last-Load',
                dataobjattr   =>'lnknfsnasipnet.srcload'),
                                                   
      new kernel::Field::CDate(
                name          =>'cdate',
                group         =>'source',
                label         =>'Creation-Date',
                dataobjattr   =>'lnknfsnasipnet.createdate'),
                                                
      new kernel::Field::MDate(
                name          =>'mdate',
                group         =>'source',
                label         =>'Modification-Date',
                dataobjattr   =>'lnknfsnasipnet.modifydate'),
                                                   
      new kernel::Field::Editor(
                name          =>'editor',
                group         =>'source',
                label         =>'Editor Account',
                dataobjattr   =>'lnknfsnasipnet.editor'),
                                                  
      new kernel::Field::RealEditor(
                name          =>'realeditor',
                group         =>'source',
                label         =>'real Editor Account',
                dataobjattr   =>'lnknfsnasipnet.realeditor'),

      new kernel::Field::Link(
                name          =>'systemnfsnasid',
                label         =>'SystemNFSNASID',
                dataobjattr   =>'lnknfsnasipnet.systemnfsnas'),
   ...
   $self->{use_distinct}=1;
   $self->setDefaultView(qw(linenumber name systemnfsnasserver 
                            systemnfsnas cdate));
   $self->setWorktable("lnknfsnasipnet");
   return($self);
}
...
{
   my $self=shift;
   my $from="lnknfsnasipnet left outer join systemnfsnas ".
            "on lnknfsnasipnet.systemnfsnas=systemnfsnas.id".
...
sub Validate
{
   ...
   if ($name eq ""){
      $self->LastMsg(ERROR,"no ip defined");
      return(undef);
   }
   else{
      $name=~s/\s//g;
      ...
      if (my ($o1,$o2,$o3,$o4,$nm)=$name=~
         ...
         if (($o1<0 || $o1 >255 ||
            ...
            $self->LastMsg(ERROR,
                           "invalid IPV4 address definition (f.e. 1.2.3.4/8)");
            return(0);
         }
      }
      else{
