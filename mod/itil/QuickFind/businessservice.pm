package itil::QuickFind::businessservice;
#  W5Base Framework
...
sub CISearchResult
{
   ...
   if (grep(/^ci$/,@$stag) &&
      ...
      foreach my $rec ($dataobj->getHashList(qw(name nature))){
         my $nature=$self->getParent->T('nat.'.$rec->{nature},
                                        "itil::businessservice");
         my $dispname=$nature.' - '.$rec->{name};

         push(@l,{group=>$self->getParent->T("itil::businessservice",
                                             "itil::businessservice"),
                  id=>$rec->{id},
                  parent=>$self->Self,
                  name=>$dispname});
...
sub QuickFindDetail
{
   ...
   my ($rec,$msg)=$dataobj->getOnlyFirst(qw(id databoss funcmgr
                                       cistatus description));
   $dataobj->ResetFilter();
   ...
   if (defined($rec)){
      ...
      $htmlresult.="<table>";
      my @l=qw(databoss funcmgr cistatus description);
      foreach my $v (@l){
         if ($rec->{$v} ne ""){
