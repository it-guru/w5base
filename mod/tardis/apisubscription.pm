package tardis::apisubscription;
#  W5Base Framework
...
sub new
{
   ...
   $self->setDefaultView(qw(applicationId name approvalstatus basepath));
   return($self);
}
...
sub DataCollector
{
   ...
   my ($restFinalAddr,$requesttoken,$constParam)=$self->Filter2RestPath(
      "/applications/{applicationId}/apisubscriptions",  # Path-Templ with var
      $filterset,
