...
use constant MYSERVER_CLIENT_VERSION41	=> 13;
...
sub protocolVersion {
	my $myserver = shift;
	return("4.1") if ($myserver->isProt41);
	return("4.0");
}

sub isProt41 {
	my $myserver = shift;
	return $myserver->[MYSERVER_CLIENT_VERSION41];
}
...
#### $field_type
	...
	my $payload;
	if ($myserver->isProt41){
		$payload = join('', map { $myserver->_lengthCodedString($_) } (
			$field_catalog, $field_db, $field_table,
			$field_org_table, $field_name, $field_org_name
		));
...
### Reading Client Hello...
	...
	my $client_flags_base = substr($data, $ptr, 2);
	$ptr = $ptr + 2;

	my $is41 = 0;  # calculating the client version from 2nd bit of 2nd byte
	$is41 = 1 if ( ord(substr($client_flags_base,1,1)) & 0x02 );
	$myserver->[MYSERVER_CLIENT_VERSION41]=$is41;

	if ($myserver->isProt41) {	
		my $client_flags      = substr($data, $ptr, 2);
		$ptr = $ptr + 2;
	}
	...
	if ($myserver->isProt41){	
		...
		my $max_packet_size = substr($data, $ptr, 3);
		$ptr = $ptr + 3;
	}
   ...
   else{	                                     # 4.0 scramble_buff always 8 bytes
		my $scramble_buff = substr($data, $ptr, 8);
		$ptr = $ptr + 8;
		$myserver->[MYSERVER_SCRAMBLE] = $scramble_buff;
	}
...
printf STDERR ("stage1=%s \n",$myserver->_xor($result, $stage1));
...
protocol 4.0 patch: Hartmut Vogler, E<lt>it@guru.de<gt> 
