...
var SOAPClient = function(mode,baseurl){
   if (mode!="public"){
	...
	this.SendRequest=function(soapReq,getResponse, callback) {		
		if(this.Proxy != null) {
         ...
         if (callback){
            ...
            acall.async=false;
            var res=getResponse($.ajax(acall));
            return(res);
         }
      }
	}
...
function getConfig(mode,baseurl)
{
...
var W5baseObject = function(config,module){
   ...
   this.findRecord=function(view,processCallback){
      ...
      function getfindRecordResponse(xData) {
         ...
         var jsOut = $.xml2json(xData.responseXML);
         if (jsOut.Body){
            jsOut=jsOut.Body;
         }
   ...
   this.getHashList=this.findRecord;
